...
public class TeacherController {
	...
	public ResponseEntity<?> crateTeacher(@RequestBody Teacher teacher, UriComponentsBuilder uriComponentsBuilder) {
	...
	public ResponseEntity<byte[]> uploadTeacherImage(@RequestParam("id_teacher") Long idTeacher,
			@RequestParam("file") MultipartFile multipartFile, UriComponentsBuilder uriBucomponentsBuilder) {
		if (idTeacher == null) {
		...
		if (teacher == null) {
			return new ResponseEntity(
					new CustomErrorType("Teacher with id_teacher: " + idTeacher.toString() + " not found"),
		...
		try {
			...
			e.printStackTrace();
			return new ResponseEntity(
					new CustomErrorType("Error durong upload: " + multipartFile.getOriginalFilename()),
					HttpStatus.NO_CONTENT);
	...
	public ResponseEntity<?> deleteTeacherImage(@PathVariable("id_teacher") Long idTeacher) {
		...
		if (teacher.getAvatar().isEmpty() || teacher.getAvatar() == null) {
			return new ResponseEntity(new CustomErrorType("Teacher does not have image assigned"),
					HttpStatus.NO_CONTENT);
		}
		String fileName = teacher.getAvatar();
		Path path = Paths.get(fileName);
		...
		if (teacher.getTeacherSocialMedia().size() == 0) {
			...
			while (i.hasNext()) {
				...
				if(teacherSocialMedia.getSocialMedia().getIdSocialMedia() == null || teacherSocialMedia.getNickname() == null) {
					return new ResponseEntity(new CustomErrorType("We need almost id_teacher, id_social_media, and nickname"),
							HttpStatus.NO_CONTENT);
				}else {
					TeacherSocialMedia tsmAux = _socialMediaService.findBySocialMediaByIdAndName(
